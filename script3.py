...
def generate_wordcloud(train_data):
    ...
    if tweets_filtrés.empty:
        st.warning("Aucun tweet disponible pour la combinaison sélectionnée.")
    else:
        # Convertir le texte en minuscules pour éviter les répétitions dues à la casse
        texte_tweets = " ".join(tweets_filtrés['Product_Description'].str.lower())

        # Compter la fréquence des mots dans les tweets filtrés
        mots_comptés = Counter(texte_tweets.split())
        ...
        wordcloud = WordCloud(width=800, height=400, max_words=nombre_mots_affichage, background_color='white', relative_scaling=0.5).generate_from_frequencies(mots_comptés)
        ...
        # Texte expliquant l'analyse du nuage de mots
        text = """
